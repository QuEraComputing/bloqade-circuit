...
from bloqade.squin.wire import Unwrap, WireType, NonDestructiveMeasure
...
def test_non_destructive_measure():
    # Create a method to test NonDestructiveMeasure
    stmts = [
        (n_qubits := as_int(1)),
        ...
        (and_measure := NonDestructiveMeasure(input_wire=wire.result)),
        (func.Return(and_measure.results[0])),
    ]

    constructed_method = gen_func_from_stmts(stmts)
    # assert return type of the method is types.Int
    assert constructed_method.return_type == types.Int
