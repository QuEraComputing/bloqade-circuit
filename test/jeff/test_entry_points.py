...
def _typed_program():
    block, (flag, count, angle) = entry(types.Bool, types.Int, types.Float)
    return method(
        block,
        (flag, count, angle),
        types.Generic(tuple, types.Bool, types.Int, types.Float),
        inputs=(types.Bool, types.Int, types.Float),
    )


def test_emit_takes_a_jeff_program_and_a_tuple_of_arguments():
    ...
    with pytest.raises(TypeError, match="args must be a tuple"):
        emit_jeff(bell(), args=[1])  # type: ignore[arg-type]


def test_specialized_arguments_become_constants():
    module = emit_jeff(_typed_program(), args=(True, 2, 0.5))
    assert module[0].body.sources == []
    assert [op.subkind for op in module[0].body.operations][:3] == [
        "const1",
        "const32",
        "const64",
    ]
...
def test_specialized_arguments_are_checked_against_the_signature():
    with pytest.raises(TypeError, match=r"args\[0\] is 1 where the program's"):
        emit_jeff(_typed_program(), args=(1,))
    with pytest.raises(TypeError, match=r"args\[0\] is 'x'"):
        emit_jeff(_typed_program(), args=("x",))
    with pytest.raises(ValueError, match="more args than kernel parameters"):
        emit_jeff(_typed_program(), args=(True, 2, 0.5, 4))
...
def test_written_bytes_do_not_depend_on_the_hash_seed(tmp_path):
    script = (
        "import sys\n"
        "from bloqade.jeff import emit_jeff\n"
        "from test.jeff.test_roundtrip import bell\n"
        "emit_jeff(bell()).write_out(sys.argv[1])\n"
    )
    root = Path(__file__).parents[2]
    outputs = []
    for seed in ("1", "2"):
        out = tmp_path / f"bell_{seed}.jeff"
        subprocess.run(
