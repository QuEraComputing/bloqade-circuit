...
def test_rejects_int64():
    ...
    with pytest.raises(JeffImportError, match="bitwidth: 64"):
...
def test_rejects_unsupported_float_bitwidth():
    ...
    with pytest.raises(JeffImportError, match="float bitwidth"):
        load_jeff(module)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            _op("intArray", "const32", jf.JeffValue(jf.IntArrayType(32, 1)), data="x"),
            "carries no values",
        ),
        (
            _op("intArray", "const32", jf.JeffValue(jf.IntArrayType(32, 1)), data=7),
            "carries no values",
        ),
        (
            _op(
                "intArray", "const32", jf.JeffValue(jf.IntArrayType(32, 1)), data=["a"]
            ),
            "carries a value that is no number",
        ),
    ],
...
def test_rejects_a_module_that_was_never_refreshed():
    value = jf.JeffOp("int", "const32", [], [jf.JeffValue(jf.IntType(32))], 1)
    body = jf.JeffRegion(sources=[], targets=list(value.outputs), operations=[value])
    module = jf.JeffModule([jf.FunctionDef(name="f", body=body)])
    with pytest.raises(JeffImportError, match="no id"):
        load_jeff(module)


def test_rejects_an_array_read_that_produces_no_integer():
    ...
    with pytest.raises(JeffImportError, match="produces no integer"):
