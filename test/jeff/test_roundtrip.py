...
and re-emitting reproduces the encoding byte for byte."""
...
def roundtrips(module) -> bool:
    """emit -> write -> load -> emit -> write is byte-identical."""
    with tempfile.TemporaryDirectory() as tmp:
        a, b = Path(tmp) / "a.jeff", Path(tmp) / "b.jeff"
        module.write_out(str(a))
        emit_jeff(load_jeff(str(a))).write_out(str(b))
...
def test_specialized_argument_becomes_a_jeff_constant():
    """`emit_jeff(method, args=...)` fixes leading parameters as jeff
    constants; the module has no inputs and roundtrips."""
    block, [n] = entry(types.Int)
    reg = add(block, stmts.RegAlloc(n)).result
    add(block, stmts.RegFree(reg))
    sized = method(block, None, types.NoneType, inputs=(types.Int,), name="sized")
    module = emit_jeff(sized, args=(3,))
    assert "int.const32" in str(module) and "in :\n" in str(module)
...
def test_roundtrips(build):
    method = build()
    validate(method)
    assert roundtrips(emit_jeff(method))
