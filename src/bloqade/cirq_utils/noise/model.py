...
class GeminiOneZoneNoiseModel_scaled(GeminiOneZoneNoiseModel):
    """
    A helper class to scale the default noise parameters of the GeminiOneZoneNoiseModel.
    """

    def __init__(
        self,
        scal_global_depolarization: float,
        scal_local_depolarization: float,
        scal_mover_pauli_probabilities: float,
        scal_sitter_pauli_probabilities: float,
        scal_paired_cz_1q_probabilities: float,
        scal_unpaired_cz_1q_probabilities: float,
    ):

        # global_depolarization_probability: float = TwoRowZoneModel().global_errors[0] * 3,
        # local_depolarization_probability: float = TwoRowZoneModel().local_errors[0] * 3,
        # mover_pauli_probabilities: Sequence[float] = (TwoRowZoneModel().mover_px, TwoRowZoneModel().mover_py,
        #                                              TwoRowZoneModel().mover_pz),
        # sitter_pauli_probabilities: Sequence[float] = TwoRowZoneModel().sitter_errors,
        # paired_cz_1q_probabilities: Sequence[float] = TwoRowZoneModel().cz_paired_errors,
        # unpaired_cz_1q_probabilities: Sequence[float] = TwoRowZoneModel().cz_unpaired_errors

        global_depolarization_probability = (
            scal_global_depolarization * TwoRowZoneModel().global_errors[0] * 3
        )
        local_depolarization_probability = (
            scal_local_depolarization * TwoRowZoneModel().local_errors[0] * 3
        )
        mover_pauli_probabilities = [
            scal_mover_pauli_probabilities * TwoRowZoneModel().mover_px,
            scal_mover_pauli_probabilities * TwoRowZoneModel().mover_py,
            scal_mover_pauli_probabilities * TwoRowZoneModel().mover_pz,
        ]
        sitter_pauli_probabilities = [
            scal_sitter_pauli_probabilities * TwoRowZoneModel().sitter_errors[0],
            scal_sitter_pauli_probabilities * TwoRowZoneModel().sitter_errors[1],
            scal_sitter_pauli_probabilities * TwoRowZoneModel().sitter_errors[2],
        ]
        paired_cz_1q_probabilities = [
            scal_paired_cz_1q_probabilities * TwoRowZoneModel().cz_paired_errors[0],
            scal_paired_cz_1q_probabilities * TwoRowZoneModel().cz_paired_errors[1],
            scal_paired_cz_1q_probabilities * TwoRowZoneModel().cz_paired_errors[2],
        ]
        unpaired_cz_1q_probabilities = [
            scal_unpaired_cz_1q_probabilities * TwoRowZoneModel().cz_unpaired_errors[0],
            scal_unpaired_cz_1q_probabilities * TwoRowZoneModel().cz_unpaired_errors[1],
            scal_unpaired_cz_1q_probabilities * TwoRowZoneModel().cz_unpaired_errors[2],
        ]

        # print("mov probs are:", mover_pauli_probabilities)
