...
def _region(
    ...
    with frame.scope() as operations:
        yields = emit.frame_call_region(frame, stmt, region, *sources)
    ...
    region_def = JeffRegion(
        sources=sources, targets=list(yields), operations=operations
    )
...
class _Scf(interp.MethodTable):
    ...
    def for_(
        ...
        region, yields = _region(emit, frame, stmt, stmt.body, sources)
    ...
    def switch(
        ...
        for branch in stmt.branches:
            ...
            region, _ = _region(emit, frame, stmt, branch, sources)
            branch_regions.append(region)
        sources = [JeffValue(value.type) for value in inputs]
        default_region, default_yields = _region(
            emit, frame, stmt, stmt.default, sources
        )
    ...
    def while_(
        ...
        sources = [JeffValue(value.type) for value in inputs]
        before_region, before_yields = _region(emit, frame, stmt, stmt.before, sources)
        outputs = before_yields[1:]
        sources = [JeffValue(value.type) for value in outputs]
        after_region, _ = _region(emit, frame, stmt, stmt.after, sources)
